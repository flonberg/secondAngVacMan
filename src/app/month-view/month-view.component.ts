...
export class MonthViewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var date = new Date('2020-11-05');                                                        // set the date, done by queryParam
   ...
   // date.setMonth(date.getMonth() - 1);
    ...
    if (dowFD == 6)
      tmpDate =  new Date(date.getFullYear(), date.getMonth(), 2);                            // if firstDayOfMonth is Sat. increment by 2 days
    if (dowFD == 0)
      tmpDate =  new Date(date.getFullYear(), date.getMonth(), 1);                            // if firstDayOfMonth is Sub. increment by 1 day
    ///////////  make the suceeding weeks  \\\\\\\\\\\\\\\\\\\\\\\\\\ 
    for (let i=1; i < 5; i++){                                                                // max of 4 more weeks in calendar
